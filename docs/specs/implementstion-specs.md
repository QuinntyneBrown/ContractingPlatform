...
### 1.1 Purpose
This Software Requirements Specification (SRS) document provides a comprehensive description of the technical requirements and architectural specifications for the ContractingPlatform system. It defines the structural organization, naming conventions, and implementation standards that shall be followed throughout the development lifecycle.

### 1.2 Scope
This specification covers the backend (.NET-based) and frontend (Angular-based) components of the ContractingPlatform system, including project structure, architectural patterns, coding conventions, and technology stack requirements.
...
The following architectural constraints are fundamental to the ContractingPlatform system and SHALL NOT be violated under any circumstances:

1. **No Repository Pattern**: ContractingPlatform SHALL NOT use the Repository pattern (IRepository or any repository abstraction) for data persistence. All data access SHALL be performed directly through the `IContractingPlatformContext` interface.

2. **Services in Core (PREFERRED)**: Business logic and domain services SHALL be implemented in `ContractingPlatform.Core` whenever possible. This is the **PREFERRED** architectural approach. Services should only be placed outside the Core project when they have explicit infrastructure dependencies.

3. **IContractingPlatformContext as Persistence Surface**: The `IContractingPlatformContext` interface is the single, unified persistence surface for the entire system. No additional abstraction layers SHALL be introduced.
...
- ContractingPlatform.Core
- ContractingPlatform.Infrastructure
- ContractingPlatform.Api
...
**REQ-SYS-006**: The system SHALL use the `IContractingPlatformContext` interface directly for all data access operations. This interface provides the persistence surface for the entire system.
